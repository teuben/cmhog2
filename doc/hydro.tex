...
In addition there will be centering (xpos,ypos,vsys)
...
is somewhat taylored for the way we measure angles in astronomy. In 
here {\tt phi} is defined as the angle between bar and disk, as
...
The best way to obtain {\tt cmhog} is via git:
...
If compilation still fails, e.g. due to the CFITSIO interface, you may also manually need to edit
the Makefile to the correct path of where CFITSIO (normally libcfitsio.a ) is located. Also
...
It is also possible that your install fails due to an HDF4 problem (note that CMHOG2
uses the older version 4 of HDF, not HDF5).
Recently the binaries that
NCSA provides (e.g. for version 4.2.8) were compiled with {\tt SZIP} compression enabled, 
although the default in the source code install is that it is disabled. If you see
an undefined reference to {\tt SZ\_encoder\_enabled}, you will either have to install
the SZIP library, or re-install HDF4 with {\tt configure --without-szlib}.  If you have
NEMO installed, look at {\tt \$NEMO/src/scripts/hdf.install}.  
As an example, for Ubuntu 12.04LTS, the system provided HDF4 libraries in /usr/lib work.
On a bind, you can use a version of HDF4 we keep in our CVS repository. See the Makefile
for details, but essentially the command would be to {\tt make hdf4} from the {\tt cmhog2}
directory.
...
{\tt cmhog} uses a file {\tt configure.ac}\footnote{older versions of autoconf
used {\tt configure.in}}.  The program {\tt autoconf} produces a new
version of {\tt configure} from this file, might you need an updated version. The {\tt configure}
script will then transform the {\tt Makefile.in} and  {\tt cmhog.def.in} into their
working counterpart. You can see,  by editing files such a {\tt Makefile} and {\tt cmhog.in}
you run the risk of loosing your work on a subsequent installation.
...
Machine & MHz  	      
	...
	& compiler/options \\ 
&&&&&  \\ \hline
i7-1185G7       & 4800  &         1.50 & 7.2  & 2944 & gfortran -g -O2 \\ % DL5420 @ 2023
i7-3820         & 3600  &         3.39 & 12.2 & 1303 & gfortran -g -O2 \\ % inferno @ 2014
i7-3630QM       & 2400  &         3.58 & 8.6  & 1235 & gfortran -g -O2 \\ % nemo2 @ 2014

Intel i7 870    & 2930  &         5.22 &      & 846  & g77 -g -O2 \\ % chara @ 2011
                &       &         4.76 &      & 928  & \\
